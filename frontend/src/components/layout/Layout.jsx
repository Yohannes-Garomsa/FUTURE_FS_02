...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-72 h-screen sticky top-0 bg-white border-r border-gray-100 pb-8 overflow-y-auto z-50">
        <div className="p-8 pb-12">
           <div className="flex items-center gap-3">
              ...
              <div>
                 <h1 className="text-xl font-black text-gray-900 leading-none tracking-tighter">FUTURE <span className="text-indigo-600">CRM</span></h1>
                 <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Enterprise Suite</p>
              </div>
           </div>
        </div>

        <nav className="flex-1 px-4 space-y-2">
           <p className="px-4 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Operations Hub</p>
           {menuItems.map(item => (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white/70 backdrop-blur-xl border-b border-gray-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
           <button 
           ...
           <div className="hidden md:flex items-center gap-4 bg-gray-50 border border-gray-100 rounded-2xl px-4 py-2 w-96 transform transition-all focus-within:ring-4 focus-within:ring-indigo-50 leading-none">
              <Search size={18} className="text-gray-400" />
              <input 
                type="text" 
                placeholder="Search global intelligence..." 
                className="bg-transparent border-none outline-none text-sm font-medium text-gray-600 w-full placeholder:text-gray-400"
              />
           </div>

           <div className="flex items-center gap-6">
              <button className="relative p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all group">
                 <Bell size={20} />
              ...
              <div className="flex items-center gap-4 pl-6 border-l border-gray-100">
                 <div className="text-right hidden sm:block leading-tight">
                    <p className="text-[11px] font-black text-gray-900 tracking-tight">{user?.full_name}</p>
                    <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest leading-none mt-1 opacity-70">{user?.role}</p>
                 </div>
                    ...
                    <AnimatePresence>
                      {isUserMenuOpen && (
                        <motion.div 
                          ...
                          className="absolute right-0 mt-4 w-56 bg-white rounded-3xl shadow-2xl border border-gray-100 py-4 z-50 overflow-hidden ring-4 ring-black/5"
                        >
                           <div className="px-6 pb-4 border-b border-gray-50 mb-4">
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.aside 
              ...
              className="fixed top-0 bottom-0 left-0 w-[85%] max-w-sm bg-white z-[70] lg:hidden flex flex-col p-8"
            >
